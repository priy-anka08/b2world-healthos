...
Purchase order lifecycle. Model: PurchaseOrder.
...
2. Create `purchase-orders.service.ts` — plain async functions, Prisma only, no
   Express types. Every query's first `where` key must be `hospitalId`.
3. Create `purchase-orders.routes.ts` — an Express router mounted behind
   `authMiddleware`, `requireTenant`, and `requirePermission("purchase_orders", action)`.
...
5. Add the relevant `Permission` rows to `backend/seed/seed.ts` (resource =
   `"purchase_orders"`) and grant them to the roles that should have
   access.
