...
Lab test catalog, orders, sample tracking, results, AI lab report extraction (spec sections 20-22). Models: LabTest, LabOrder, LabResult.
...
2. Create `laboratory.service.ts` — plain async functions, Prisma only, no
   Express types. Every query's first `where` key must be `hospitalId`.
3. Create `laboratory.routes.ts` — an Express router mounted behind
   `authMiddleware`, `requireTenant`, and `requirePermission("lab_tests", action)`.
...
5. Add the relevant `Permission` rows to `backend/seed/seed.ts` (resource =
   `"lab_tests"`) and grant them to the roles that should have
   access.
