...
2. Create `wards.service.ts` — plain async functions, Prisma only, no
   Express types. Every query's first `where` key must be `hospitalId`.
3. Create `wards.routes.ts` — an Express router mounted behind
   `authMiddleware`, `requireTenant`, and `requirePermission("wards", action)`.
...
5. Add the relevant `Permission` rows to `backend/seed/seed.ts` (resource =
   `"wards"`) and grant them to the roles that should have
   access.
