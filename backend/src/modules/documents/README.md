...
Hospital SOP/policy documents feeding the RAG assistant (spec sections 13-14). Models: Document, DocumentChunk.
...
2. Create `documents.service.ts` — plain async functions, Prisma only, no
   Express types. Every query's first `where` key must be `hospitalId`.
3. Create `documents.routes.ts` — an Express router mounted behind
   `authMiddleware`, `requireTenant`, and `requirePermission("documents", action)`.
...
5. Add the relevant `Permission` rows to `backend/seed/seed.ts` (resource =
   `"documents"`) and grant them to the roles that should have
   access.
