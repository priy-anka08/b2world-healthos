...
Hospital equipment registry + predictive maintenance (spec sections 28-29). Models: Asset, MaintenanceRecord.
...
2. Create `assets.service.ts` — plain async functions, Prisma only, no
   Express types. Every query's first `where` key must be `hospitalId`.
3. Create `assets.routes.ts` — an Express router mounted behind
   `authMiddleware`, `requireTenant`, and `requirePermission("assets", action)`.
...
5. Add the relevant `Permission` rows to `backend/seed/seed.ts` (resource =
   `"assets"`) and grant them to the roles that should have
   access.
