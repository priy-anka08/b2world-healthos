...
Medication requests tied to an encounter. Model: MedicationRequest.
...
2. Create `prescriptions.service.ts` — plain async functions, Prisma only, no
   Express types. Every query's first `where` key must be `hospitalId`.
3. Create `prescriptions.routes.ts` — an Express router mounted behind
   `authMiddleware`, `requireTenant`, and `requirePermission("prescriptions", action)`.
...
5. Add the relevant `Permission` rows to `backend/seed/seed.ts` (resource =
   `"prescriptions"`) and grant them to the roles that should have
   access.
