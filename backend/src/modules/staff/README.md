...
Staff directory, attendance, shifts, leave. Models: Staff, Shift.
...
2. Create `staff.service.ts` — plain async functions, Prisma only, no
   Express types. Every query's first `where` key must be `hospitalId`.
3. Create `staff.routes.ts` — an Express router mounted behind
   `authMiddleware`, `requireTenant`, and `requirePermission("staff", action)`.
...
5. Add the relevant `Permission` rows to `backend/seed/seed.ts` (resource =
   `"staff"`) and grant them to the roles that should have
   access.
