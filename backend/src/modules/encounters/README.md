...
Clinical encounter lifecycle (open/close visit). Model: Encounter.
...
2. Create `encounters.service.ts` — plain async functions, Prisma only, no
   Express types. Every query's first `where` key must be `hospitalId`.
3. Create `encounters.routes.ts` — an Express router mounted behind
   `authMiddleware`, `requireTenant`, and `requirePermission("encounters", action)`.
...
5. Add the relevant `Permission` rows to `backend/seed/seed.ts` (resource =
   `"encounters"`) and grant them to the roles that should have
   access.
