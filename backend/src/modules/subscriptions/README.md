...
Subscription plans, usage limits, billing for the platform itself (commercialization). Model: Subscription.
...
2. Create `subscriptions.service.ts` — plain async functions, Prisma only, no
   Express types. Every query's first `where` key must be `hospitalId`.
3. Create `subscriptions.routes.ts` — an Express router mounted behind
   `authMiddleware`, `requireTenant`, and `requirePermission("subscriptions", action)`.
...
5. Add the relevant `Permission` rows to `backend/seed/seed.ts` (resource =
   `"subscriptions"`) and grant them to the roles that should have
   access.
