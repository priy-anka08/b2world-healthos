...
Medicine catalog, batch/expiry tracking, AI inventory prediction (spec sections 17-18). Models: InventoryItem, InventoryBatch.
...
2. Create `pharmacy.service.ts` — plain async functions, Prisma only, no
   Express types. Every query's first `where` key must be `hospitalId`.
3. Create `pharmacy.routes.ts` — an Express router mounted behind
   `authMiddleware`, `requireTenant`, and `requirePermission("pharmacy", action)`.
...
5. Add the relevant `Permission` rows to `backend/seed/seed.ts` (resource =
   `"pharmacy"`) and grant them to the roles that should have
   access.
