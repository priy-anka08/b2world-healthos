...
In-app/email/SMS notifications (appointment reminders, low stock, etc). Model: Notification.
...
2. Create `notifications.service.ts` — plain async functions, Prisma only, no
   Express types. Every query's first `where` key must be `hospitalId`.
3. Create `notifications.routes.ts` — an Express router mounted behind
   `authMiddleware`, `requireTenant`, and `requirePermission("notifications", action)`.
...
5. Add the relevant `Permission` rows to `backend/seed/seed.ts` (resource =
   `"notifications"`) and grant them to the roles that should have
   access.
